...
static int test_au(int fd, void *buffer) {
    ...
    if (hwparams.channels < 1 || hwparams.channels > 256)
        return -1;
    if ((size_t) safe_read(fd, buffer + sizeof(AuHeader), BE_INT(ap->hdr_size) - sizeof(AuHeader)) !=
...
static ssize_t pcm_write(u_char *data, size_t count) {
    ...
    if (count < chunk_size) {
        snd_pcm_format_set_silence(hwparams.format, data + count * bits_per_frame / 8,
...
static void voc_pcm_flush(void) {
    if (buffer_pos > 0) {
        size_t b;
        if (snd_pcm_format_set_silence(hwparams.format, audiobuf + buffer_pos,
